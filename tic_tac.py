...
def step (d, player, x, y):
    ...
    if x in range(1,4) and y in range(1,4):
        if Field[x][y] == 0:
            ...
            x = client_input("введите другой номер поля по вертикали: ")
            y = client_input("введите другой номер поля по горизонтали: ")
            return step(d, player, x, y)
    else:
        print("Введенные значения выходят за рамки игрового поля.")
        x = client_input("введите другой номер поля по вертикали: ")
        y = client_input("введите другой номер поля по горизонтали: ")
        return step(d, player, x, y)
...
print_field(Field)
while win_flag:
    x = client_input(f"{names[player]}, введите номер поля по вертикали: ")
    y = client_input(f"{names[player]}, введите номер поля по горизонтали: ")
